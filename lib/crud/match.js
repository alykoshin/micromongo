...
function doPostOp(doc, op, subQuery, options) {
  ...
  return operators[ op ](doc, subQuery, options); // execute it
}
...
function doExpr(doc, query) {
  ...
  if (query !== null && typeof query === 'object' && !Array.isArray(query)) {
    ...
    for (var k2 in query) { if (query.hasOwnProperty(k2)) {
      debug('doExpr(): k2: '+k2);
      if (k2.charAt(0) === '$') {
        res = doPostOp(doc, k2, query[k2], query.$options); if (!res) { debug('doExpr(): false'); return false; }
      } else { // this is a field name, not operator
        res = doPostOp( _.get(doc, k2), '$eq', query[k2], query.$options); if (!res) { debug('doExpr(): false'); return false; }
      }
    }}
