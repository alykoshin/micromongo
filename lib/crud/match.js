...
function doPreOp(doc, op, subQuery) {
  ...
  return preOperators[ op ].call(this, doc, subQuery);
}
...
var _match1 = function(doc, query) {
  ...
  for (var k1 in query) { if (query.hasOwnProperty(k1)) {
    ...
    if (k1.charAt(0) === '$') { // match if it is logical operator
      res = doPreOp.call(this, doc, k1, query[ k1 ]); if (!res) { return false; }

    } else {
      res = doExpr.call(this,  _.get(doc, k1), query[k1]); if (!res) { return false; }
    }
