...
function _contains(target, source, options) {
  debug('_contains(): target: ' + util.inspect(target) + ', source: ' + util.inspect(source) + ', options: ' + util.inspect(options));
  ...
  if (['undefined', 'boolean', 'number', 'string'].indexOf(typeof target)>=0 || target === null ) {
    debug('_contains(): ('+util.inspect(target)+'==='+util.inspect(source)+') = ' + (target === source));
...
function _containsAllDeep(target, source, options) {
  options = options || {};
  options.all  = true;
  debug('_containsAllDeep: target: '+util.inspect(target)+', source: '+util.inspect(source));
...
function _containsAnyDeep(target, source, options) {
  options = options || {};
  options.any  = true;
  debug('_containsAnyDeep: target: '+util.inspect(target)+', source: '+util.inspect(source));
  return _contains(target, source, options);
}
...
var _eql = function(target, source, options) {
  options = options || {};
  debug('_eql: target: '+util.inspect(target)+', source: '+util.inspect(source));
...
var _arrayEql = function(target, source, options) {
  options = options || {};
  debug('_arrayEql: target: '+util.inspect(target)+', source: '+util.inspect(source)+', options: '+util.inspect(options));
...
var _arrayElementEql = function(target, source, options) {
  options = options || {};
  debug('_arrayEqlOrElementEql: target: '+util.inspect(target)+', source: '+util.inspect(source)+', options: '+util.inspect(options));
...
var preOperators = {
  ...
  $and: function(doc, query) {
    debug('$and: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $and must be array.');
    // iterate the array
    for (var len=query.length, i=0; i<len; ++i) {
      var q1 = query[i];
      var res = _match1.call(this, doc, q1);
      debug('$and: q1: ' + util.inspect(q1) + ', res: ' +util.inspect(res));
      if (!res) {
        return false;
      }
    }
  ...
  $or: function(doc, query) {
    debug('$or: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $or must be array.');
    // iterate the array
    for (var len=query.length, i=0; i<len; ++i) {
      var q1 = query[i];
  ...
  //  //console.log('$not: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
  ...
  $nor: function(doc, query) {
    //console.log('$nor: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $nor must be array.');
    // iterate the array
    for (var len=query.length, i=0; i<len; ++i) {
      var q1 = query[i];
      //console.log('$or: q1', q1);
  ...
  $where: function(doc, query) {
    ...
    //if (!whereCache[query]) {
      ...
      if (typeof query === 'string') {
        ...
        throw new TypeError('Invalid syntax near \'' + util.inspect(query) + '\': parameter for $where must be string or function.');
      }
...
var postOperators = {
  ...
  $not: function(doc, query) {
    debug('$not: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
  ...
  $eq: function(doc, query) {
    debug('$eq: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
  ...
  $ne: function(doc, query) {
    debug('$ne: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    return doc !== query;
  },

  $gt: function(doc, query) {
    debug('$gt: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    return doc > query;
  },

  $gte: function(doc, query) {
    debug('$gte: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    return doc >= query;
  },

  $lt: function(doc, query) {
    debug('$lt: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    return doc < query;
  },

  $lte: function(doc, query) {
    debug('$lte: doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    return doc <= query;
  },

  $in: function(doc, query) {
    debug('$in(): doc: '+util.inspect(doc)+', query: '+util.inspect(query));
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $in must be array.');
  ...
  $nin: function(doc, query) {
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $nin must be array.');
  ...
  $exists: function(doc, query) {
    assert(typeof query === 'boolean', 'Invalid syntax near \'' + util.inspect(query) + '\': parameter for $exists must be boolean.');
    var q = query;
    return (
  ...
  $type: function(doc, query) {
    ...
    if ( typeof query !== 'string' || allowedDataTypes.indexOf(query) < 0) {
      throw new Error('Invalid syntax near \''+util.inspect(query)+'\': parameter for $type must be one of following types: '+allowedDataTypes.join(', ')+'.');
    }
    return (
      (query === typeof doc) ||
      (query === 'null'  && doc === null) ||
  ...
  $mod: function(doc, query) {
    assert(Array.isArray(query) && query.length === 2, 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $in must be array with length = 2');
    var divisor = query[0];
  ...
  $regex: function(doc, query, options) {
    assert(query instanceof RegExp, 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $regex must be regex');
    //console.log(util.inspect(doc), query.toString(), options)
  ...
  $all: function (doc, query) {
    assert(Array.isArray(query), 'Invalid syntax near \''+util.inspect(query)+'\': parameter for $all must be array.');
  ...
  $size: function (doc, query) {
    if ( typeof query !== 'number') {
      throw new Error('Invalid syntax near \''+util.inspect(query)+'\': parameter for $size must be a number.');
    }
...
function doPreOp(doc, op, subQuery, options) {
  if (typeof preOperators[ op ] === 'undefined') { // execute it
    throw new Error('Invalid operator \''+util.inspect(op));
  }
  return preOperators[ op ].call(this, doc, subQuery, options);
}

function doPostOp(doc, op, subQuery, options) {
  debug('doPostOp(): doc: '+util.inspect(doc)+', op:'+util.inspect(op)+', subQuery:'+util.inspect(subQuery)+', options:'+util.inspect(options));
...
function doExpr(doc, query, options) {
  debug('doExpr(): doc: '+util.inspect(doc)+', query:'+util.inspect(query));
  var res;
  // as this is not logical operator, this is a field name
  // check if there is an operator inside
  if (query === null) {
    debug('doExpr(): true (query is empty)');
    return true;
    ...
    for (var k2 in query) { if (query.hasOwnProperty(k2)) {
      ...
      if (k2.charAt(0) === '$') {
        ...
        if (!res) { debug('doExpr(): false'); return false; }
  ...
  debug('doExpr(): true (default)');
  return true;
}

var _match1 = function(doc, query) {
  debug('* _match1(): doc: '+util.inspect(doc)+', query: '+util.inspect(query));
  var res;
...
var _match0 = function(doc, query) {
  debug('* _match0(): doc: '+ util.inspect(doc)+', query: '+ util.inspect(query));
...
var match = function(doc, query) {
  debug('* match(): doc: '+ util.inspect(doc)+', query: '+ util.inspect(query));
...
var prepareQuery = function(query) {
  debug('* prepareQuery(): query: '+ util.inspect(query));
