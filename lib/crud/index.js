 ...
 */
var distinct = function(array, field, query) {
...
// contributes 0. Upsert is not implemented (no upsertedId/upsertedCount).

var updateOne = function(array, query, update, options) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
  ...
  for (var m of matches(array, query)) { // lazy: first match only
    matchedCount = 1;
    if (applyUpdate(m.doc, update)) { modifiedCount = 1; }
    break;
  }
...
var updateMany = function(array, query, update, options) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
  if (!applyUpdate.hasOperators(update)) {
  ...
  for (var i = 0; i < docs.length; ++i) {
    if (applyUpdate(docs[i], update)) { modifiedCount++; }
  }
  return { acknowledged: true, matchedCount: docs.length, modifiedCount: modifiedCount };
};


var replaceOne = function(array, query, replacement, options) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
...
var findOneAndUpdate = function(array, query, update, options) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
  for (var m of matches(array, query)) {
    var before = deepAssign({}, m.doc);
    applyUpdate(m.doc, update);
    return before;
  }
  return null;
};
...
var findOneAndReplace = function(array, query, replacement, options) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
  if (applyUpdate.hasOperators(replacement)) {
...
var findOneAndDelete = function(array, query) {
  if (!Array.isArray(array)) { throw new TypeError('Array expected as first parameter'); }
  for (var m of matches(array, query)) {
    var doc = array[m.i];
    array.splice(m.i, 1);
