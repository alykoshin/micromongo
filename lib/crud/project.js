...
var project = function(doc, projection) {
  ...
  var validateProjectionValue = function(value) {
    if (allowedProjectionValues.indexOf(value) < 0) {
      throw new Error('Values for projection musty be in ' + JSON.stringify(allowedProjectionValues) + ', found: '+util.inspect(value));
    }
  };
  ...
  for (path in projection) { if (projection.hasOwnProperty(path)) {
    ...
    validateProjectionValue(projection[ path ]);
    //
    // Handle _id similar to Mongo
