 ...
 *       matching, simple term-frequency score. Zero linguistic processing. Cheapest
 *       and dependency-free; best fit for the library's "small array" identity.
 *
...
function porterStem(w) {
  ...
  if (re.test(w)) { var fp = re.exec(w); re = new RegExp(mgr0); if (re.test(fp[1])) { re = /.$/; w = w.replace(re, ''); } }
    ...
    var fp2 = re2.exec(w); stem = fp2[1]; re2 = new RegExp(s_v);
    if (re2.test(stem)) {
      w = stem;
  ...
  // Step 1c
  re = /^(.+?)y$/;
  if (re.test(w)) { var fp1c = re.exec(w); stem = fp1c[1]; re = new RegExp(s_v); if (re.test(stem)) { w = stem + 'i'; } }

  // Step 2
  re = /^(.+?)(ational|tional|enci|anci|izer|bli|alli|entli|eli|ousli|ization|ation|ator|alism|iveness|fulness|ousness|aliti|iviti|biliti|logi)$/;
  if (re.test(w)) { var fp2b = re.exec(w); stem = fp2b[1]; suffix = fp2b[2]; re = new RegExp(mgr0); if (re.test(stem)) { w = stem + step2list[suffix]; } }

  // Step 3
  re = /^(.+?)(icate|ative|alize|iciti|ical|ful|ness)$/;
  if (re.test(w)) { var fp3 = re.exec(w); stem = fp3[1]; suffix = fp3[2]; re = new RegExp(mgr0); if (re.test(stem)) { w = stem + step3list[suffix]; } }

  // Step 4
  re = /^(.+?)(al|ance|ence|er|ic|able|ible|ant|ement|ment|ent|ou|ism|ate|iti|ous|ive|ize)$/; re2 = /^(.+?)(s|t)(ion)$/;
  if (re.test(w)) { var fp4 = re.exec(w); stem = fp4[1]; re = new RegExp(mgr1); if (re.test(stem)) { w = stem; } }
  else if (re2.test(w)) { var fp4b = re2.exec(w); stem = fp4b[1] + fp4b[2]; re2 = new RegExp(mgr1); if (re2.test(stem)) { w = stem; } }

  // Step 5
  re = /^(.+?)e$/;
  if (re.test(w)) { var fp5 = re.exec(w); stem = fp5[1]; re = new RegExp(mgr1); re2 = new RegExp(meq1); re3 = new RegExp('^' + C + v + '[^aeiouwxy]$'); if (re.test(stem) || (re2.test(stem) && !re3.test(stem))) { w = stem; } }
 ...
 */
function parseSearch(search, m) {
  var terms = [], phrases = [], negations = [];
  var re = /"([^"]+)"|(\S+)/g, match;
...
function relevance(totalTermFreq, hitCount, numTokens, m) {
...
module.exports.porterStem = porterStem;
