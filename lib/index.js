...
var operators = {
  ...
  $in: function(doc, query) {
    ...
    if ( !Array.isArray(query) ) {
      throw new Error('Invalid syntax near \''+JSON.stringify(query,null,2)+'\': parameter for $in must be array.');
    }
  ...
  $nin: function(doc, query) {
    //console.log('$in: doc: '+JSON.stringify(doc)+', query: '+JSON.stringify(query));
    if ( !Array.isArray(query) ) {
      throw new Error('Invalid syntax near \''+JSON.stringify(query,null,2)+'\': parameter for $nin must be array.');
    }
    //console.log('$eq: doc:', doc, ', query:', query);
    var q = query;//calcArr(doc,query);
    return (q.indexOf(doc) < 0);
  ...
  $exists: function(doc, query) {
    ...
    if (typeof query !== 'boolean') {
      throw new Error('Invalid syntax near \'' + JSON.stringify(query, null, 2) + '\': parameter for $exists must be boolean.');
    }
...
function projectionMode(projection) {
  for (var path in projection) { if (projection.hasOwnProperty(path)) {
    //
    // no special handling for _id field
...
var project = function(doc, projection) {
  ...
  if (mode === 'exclusion') { // exclusion mode
    ...
    for (path in projection) { if (projection.hasOwnProperty(path)) {
      if (path === '_id') { // ignore setting
        continue;
      } else if (projection[ path ] !== false && projection[ path ] !== 0) {
        throw new Error('Values for projection in exclusion mode may be false or 0');
      }
    ...
    for (path in projection) { if (projection.hasOwnProperty(path)) {
      if (path === '_id') { // ignore setting
        continue;
      } else if (projection[path] !== true && projection[path] !== 1 ) {
        throw new Error('Values for projection in inclusion mode may be true or 1');
      }
...
var find = function(array, query, projection) {
  return array
    .filter(function(doc) { return match(doc, query); })
    .map(function(doc) { return project(doc, projection)})
    ;
};


var findOne = function(array, query) {
  var arr = find(array, query);
  if (arr.length >= 0 ) {
    return arr[0];
  }
  return null;
