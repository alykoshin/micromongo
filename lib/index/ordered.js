...
/**
 * @param {Object} spec - { field: 1, … } (1 = ascending; we only support ascending
 *                         keys — descending sort is served by reversing the slice).
 */
function OrderedIndex(spec) {
  if (typeof spec === 'string') { var s = {}; s[spec] = 1; spec = s; }
  this.spec = spec;
...
OrderedIndex.prototype._keyFromValues = function (values) {
 ...
 */
OrderedIndex.prototype.build = function (docs) {
  var entries = [];
  this.multiKey = false;
  ...
  entries.sort(function (a, b) { return cmp.compareKeys(a.key, b.key); });
 ...
 */
OrderedIndex.prototype._buildHash = function (entries) {
...
OrderedIndex.prototype._emitEntries = function (doc, entries) {
  var self = this;
  ...
  var values = this.fields.map(function (f) { return _.get(doc, f); });
...
OrderedIndex.prototype._lowerBound = function (key) {
...
/** First index with entries[i].key > key. */
OrderedIndex.prototype._upperBound = function (key) {
  var lo = 0, hi = this.entries.length;
  while (lo < hi) {
    var mid = (lo + hi) >>> 1;
    if (cmp.compareKeys(this.entries[mid].key, key) <= 0) { lo = mid + 1; } else { hi = mid; }
...
OrderedIndex.prototype._collect = function (start, end) {
  var out = [];
 ...
 * @returns {Array<doc>}
 */
OrderedIndex.prototype.eqRange = function (key) {
  this.hits++;
 ...
 * @returns {Array<doc>}
 */
OrderedIndex.prototype.range = function (bounds) {
  this.hits++;
 ...
 * @returns {Array<doc>}
 */
OrderedIndex.prototype.sorted = function (dir) {
  this.hits++;
