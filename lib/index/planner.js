...
var RANGE_OPS = { $gt: 'gt', $gte: 'gte', $lt: 'lt', $lte: 'lte' };
...
/**
 * @param {Object} query
 * @param {Object} indexes  field-name(joined) → OrderedIndex (Collection._indexes)
 * @param {Function} getIndexFor  (fieldsArrayOrName) → OrderedIndex | undefined
 * @returns {{docs: Array, exact: boolean, explain: Object} | null}
 *
 ...
 */
function indexId(idx) { return idx.fields.map(function (f) { return f + '_' + idx.spec[f]; }).join('_'); }

function plan(query, indexes, getIndexFor) {
...
module.exports = { plan: plan, rangeBounds: rangeBounds, isEqualityValue: isEqualityValue };
