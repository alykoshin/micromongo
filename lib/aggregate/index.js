 ...
 * @returns {{ data: Array, set: function(Array) }} a handle to read/replace the data
 */
function _resolveCollection(ref, createIfMissing) {
  if (typeof ref === 'string') {
    ...
    return { data: coll._data, set: function (arr) { coll._data.length = 0; Array.prototype.push.apply(coll._data, arr); } };
  }
  if (ref && Array.isArray(ref._data)) { // a Collection
    var c = ref;
    return { data: c._data, set: function (arr) { c._data.length = 0; Array.prototype.push.apply(c._data, arr); } };
  }
  if (Array.isArray(ref)) {
    var a = ref;
    return { data: a, set: function (arr) { a.length = 0; Array.prototype.push.apply(a, arr); } };
  }
  return null;
}
 ...
 * @returns {string} the location field path
 */
var _geoNearDefaultKey = function(array) {
...
var _groupAccumulators = {
  ...
  $push: function(docs, expr) {
    return docs.map(function(d) { return evaluate(expr, d); });
  },
  $addToSet: function(docs, expr) {
    var out = [];
    for (var i = 0; i < docs.length; ++i) {
      var v = evaluate(expr, docs[i]);
      if (!out.some(function(x) { return _.isEqual(x, v); })) { out.push(v); }
    }
    return out;
  },
  $first: function(docs, expr) {
    return docs.length ? evaluate(expr, docs[0]) : null;
  },
  $last: function(docs, expr) {
    return docs.length ? evaluate(expr, docs[docs.length - 1]) : null;
  },
  $count: function(docs /*, expr */) {
    return docs.length;
  },
};
...
var _aggregateStageOps = {
  ...
  $project: function(array, projection, options) {
    ...
    var plain = {}, computed = {}, hasComputed = false;
  ...
  $addFields: function(array, spec, options) {
    var target = [];
  ...
  $unset: function(array, spec, options) {
  ...
  $count: function(array, fieldName, options) {
  ...
  $replaceRoot: function(array, params, options) {
  ...
  $sortByCount: function(array, expr, options) {
    var grouped = _aggregateStageOps.$group(array, { _id: expr, count: { $sum: 1 } }, options);
    return grouped.sort(function(a, b) { return b.count - a.count; });
  },


  $match: function(array, query, options) {
    const target = [];
  ...
  $lookup: function(array, params, options) {
    ...
    for (var i = 0; i < array.length; ++i) {
      ...
      for (var f = 0; f < foreign.length; ++f) {
        ...
        var hit = wanted.some(function (w) { return _.isEqual(w, fv); });
  ...
  $indexStats: function(array, params, options) {
    ...
    return stats.map(function (s) { return deepAssign({}, s); });
  },
...
_aggregateStageOps.$replaceWith = function(array, expr, options) {
