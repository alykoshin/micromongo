 ...
 * @param [options.all]
 * @param [options.any]
 * @param [options.sameOrder] - require arrays elements to have same positions
 ...
 * @returns bitmask
 */
function _bitmaskFromOperand(operand: number | number[]): number {
 ...
 * @param mode
 */
function _bitsMatch(doc: any, operand: number | number[], mode: 'allSet' | 'anySet' | 'allClear' | 'anyClear'): boolean {
 ...
 * @param operand  the $near value: [x,y] (legacy) or { $geometry, $maxDistance, $minDistance } (GeoJSON)
