 ...
 */
function OrderedIndex(this: any, spec: any) {
  if (typeof spec === 'string') { var s: any = {}; s[spec] = 1; spec = s; }
  this.spec = spec;
...
OrderedIndex.prototype._keyFromValues = function (this: any, values: any) {
 ...
 */
OrderedIndex.prototype.build = function (this: any, docs: any) {
  var entries: any[] = [];
  this.multiKey = false;
  ...
  entries.sort(function (a: any, b: any) { return cmp.compareKeys(a.key, b.key); });
 ...
 */
OrderedIndex.prototype._buildHash = function (this: any, entries: any) {
  if (this.compound) { this.hash = null; return; }
  var hash = new Map();
  for (var i = 0; i < entries.length; ++i) {
    var k = entries[i].key, doc = entries[i].doc;
    var bucket = hash.get(k);
    if (bucket) {
      // dedup: a multikey doc with repeated element values appears once per key
      if (bucket[bucket.length - 1] !== doc && bucket.indexOf(doc) === -1) { bucket.push(doc); }
    } else {
      hash.set(k, [ doc ]);
    }
  }
...
OrderedIndex.prototype._emitEntries = function (this: any, doc: any, entries: any) {
  var self = this;
  ...
  var values = this.fields.map(function (f: any) { return _.get(doc, f); });

  // find an array-valued field to expand (first one)
  var arrIdx = -1;
  for (var j = 0; j < values.length; ++j) {
    if (Array.isArray(values[j])) { arrIdx = j; break; }
  }
  ...
  if (arr.length === 0) {
    ...
    var v0 = values.slice(); v0[arrIdx] = undefined;
...
OrderedIndex.prototype._lowerBound = function (this: any, key: any) {
  var lo = 0, hi = this.entries.length;
  while (lo < hi) {
    var mid = (lo + hi) >>> 1;
    if (cmp.compareKeys(this.entries[mid].key, key) < 0) { lo = mid + 1; } else { hi = mid; }
  }
  return lo;
};

/** First index with entries[i].key > key. */
OrderedIndex.prototype._upperBound = function (this: any, key: any) {
  var lo = 0, hi = this.entries.length;
  while (lo < hi) {
    var mid = (lo + hi) >>> 1;
    if (cmp.compareKeys(this.entries[mid].key, key) <= 0) { lo = mid + 1; } else { hi = mid; }
...
OrderedIndex.prototype._collect = function (this: any, start: any, end: any) {
  var out: any[] = [];
  var seen = (typeof Set !== 'undefined') ? new Set() : null;
  for (var i = start; i < end; ++i) {
    var doc = this.entries[i].doc;
    if (seen) { if (seen.has(doc)) { continue; } seen.add(doc); }
    out.push(doc);
  }
  return out;
};
 ...
 */
OrderedIndex.prototype.eqRange = function (this: any, key: any): any[] {
  this.hits++;
 ...
 */
OrderedIndex.prototype.range = function (this: any, bounds: any): any[] {
  this.hits++;
  var start = 0, end = this.entries.length;
  if ('gte' in bounds) { start = this._lowerBound(bounds.gte); }
  else if ('gt' in bounds) { start = this._upperBound(bounds.gt); }
  if ('lte' in bounds) { end = this._upperBound(bounds.lte); }
  else if ('lt' in bounds) { end = this._lowerBound(bounds.lt); }
 ...
 */
OrderedIndex.prototype.sorted = function (this: any, dir: any): any[] {
  this.hits++;
  var docs = this._collect(0, this.entries.length);
  if (dir === -1) { docs.reverse(); }
  return docs;
};
