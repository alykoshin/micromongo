> ⚠️ **Archived — this is the README for micromongo `0.3.1`** (the last release before the
> TypeScript rewrite). It documents the old, pre-index API and is kept for historical reference only.
> **For the current 1.x version, see the [live docs & playground](https://alykoshin.github.io/micromongo/)
> and the [current README](https://github.com/alykoshin/micromongo/blob/master/README.md).**
>
> Notable differences vs. 1.x: 0.3.x had **no indexes / no `Collection`**, a smaller operator set, no
> TypeScript types, and targeted `node >= 0.11`. The dead CI/badge services from that era have been
